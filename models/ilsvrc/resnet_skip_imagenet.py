...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        layers = []
        layers.append(block(self.in_planes, planes, stride, skippable=False))
...
def ResNet34_skip(num_classes=1000):
    return ResNet_skip(BasicBlock, [3, 4, 6, 3], num_classes)

def ResNet50_skip(num_classes=1000):
    return ResNet_skip(Bottleneck, [3, 4, 6, 3], num_classes)

def ResNet101_skip(num_classes=1000):
